...
<body>
	...
	<div class="container">
		<div class="row">
			<!--导航菜单-->
			<div class="col-sm-3"><#assign currentMenu="borrowBidReturn" />
				<#include "common/leftmenu-tpl.ftl" /></div>
			<!-- 功能页面 -->
			<div class="col-sm-9">
				<form action="/borrowBidReturn_list" name="searchForm"
				...
				<div class="panel panel-default">
					...
					<input type="hidden" id="useableAmount"
						value="${account.usableAmount?string('0.##')}">
					<table class="table">
						<thead>
							<tr>
								<th>借款</th>
						...
						<tbody>
							...
							<tr>
								<td>
									<a href="/borrow_info?id=${data.bidRequestId}">${data.bidRequestTitle}</a>
								</td>
								...
								<td>
									<#if data.state=0>
									<a class="btn btn-success return_money"
										data-returnmoney="${data.totalAmount?string('0.##')}"
										data-rid="${data.id}">立刻还款</a>
									<#else> ${data.stateDisplay} </#if>
								</td>
							</tr>
							</#list>
						</tbody>
